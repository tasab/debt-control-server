...
import { resetDb, seedFeePolicy } from './helpers.js'
...
const rawCookie = (response) => [response.headers['set-cookie']].flat()[0] ?? ''
const cookieOf = (response) => rawCookie(response).split(';')[0]

async function registerUser(capability = 'invest') {
...
test('register → me → wallets, with the session in an httpOnly cookie', async () => {
  ...
  const uah = wallets.json().find((w) => w.currency === 'UAH')
...
test('a transfer moves money and shows up in both histories', async () => {
  ...
  assert.equal(recipientWallets.json().find((w) => w.currency === 'UAH').available, '100000')
