...
import { fxQuotes, rateSources } from '../db/schema/index.ts'
import { convertMinor, execute, ingestRates, parseRate, quote, RATE_SCALE } from '../src/fx/service.ts'
...
import { fund, makeUser, resetDb } from './helpers.ts'
...
test('an expired quote is refused with QUOTE_EXPIRED', async () => {
  ...
  await assert.rejects(
    ...
    (err: any) => err.code === 'QUOTE_EXPIRED',
  )
