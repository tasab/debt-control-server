...
import { currencies, feePolicies, users } from '../db/schema/index.ts'
...
export async function seedFeePolicy(
  overrides: Partial<typeof feePolicies.$inferInsert> = {},
) {
...
export async function makeUser({
  ...
  isAdmin = false,
  name = 'Тест',
}: { capabilities?: string[]; isAdmin?: boolean; name?: string } = {}) {
  const [user] = await db
    .insert(users)
    .values({
      id: newId('usr'),
