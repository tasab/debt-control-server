...
import { businesses, loanShares, loans } from '../db/schema/index.ts'
import { newId } from '../src/money/amount.ts'
import { availableBalance, balanceOf } from '../src/money/balances.ts'
import { ledgerTotals } from '../src/money/ledger.ts'
import { businessWallet, platformFee, userHold } from '../src/money/accounts.ts'
import { createRequest, cancelRequest, fundRequest } from '../src/domain/requests.ts'
import { accrueInterest, repay } from '../src/domain/loans.ts'
import { moveInternal } from '../src/domain/businesses.ts'
...
test('overfunding and sub-minimum tickets are refused', async () => {
  ...
  await assert.rejects(
    ...
    (err: any) => err.code === 'BELOW_MIN_TICKET',
  )
  await assert.rejects(
    fundRequest({
      requestId: request.id,
