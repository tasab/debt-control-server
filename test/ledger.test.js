...
import { postTransaction, ledgerTotals, computeBalance } from '../src/money/ledger.js'
import { availableBalance } from '../src/money/balances.js'
import { externalAccount, userWallet } from '../src/money/accounts.js'
import { newId } from '../src/money/amount.js'
import { transfer, topUp } from '../src/domain/transfers.js'
...
test('parallel requests sharing one Idempotency-Key post exactly once', async () => {
  ...
  const ids = new Set(results.filter((r) => r.status === 'fulfilled').map((r) => r.value.transactionId))
...
test('the database rejects an unbalanced insert even if the service is bypassed', async () => {
  ...
  await assert.rejects(
    ...
    (err) => /does not balance/.test(err.cause?.message ?? err.message),
  )
})
