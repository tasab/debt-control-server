import { z } from 'zod'
import { getLoan, listLoans, repay } from '../domain/loans.ts'
import { repaySchema } from '../validation/requests.ts'
...
const serializeLoan = (loan: Record<string, any>) => ({
  id: loan.id,
  status: loan.status,
...
export default async function loanRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<IdParams>('/loans/:id', { preHandler: fastify.authenticate }, async (request) => {
    const loan = await getLoan(request.params.id, request.user.id)
    return {
      ...serializeLoan(loan),
      business: loan.business,
      schedule: loan.schedule.map((row: Record<string, any>) => ({
        id: row.id,
        seq: row.seq,
        dueAt: iso(row.dueAt),
      ...
      investors: loan.investors.map((i: Record<string, any>) => ({
        name: i.name,
        shareBps: i.shareBps,
  ...
  fastify.post<IdParams>('/loans/:id/repay', { preHandler: fastify.guard(['borrow']) }, async (request, reply) => {
