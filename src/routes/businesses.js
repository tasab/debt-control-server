import * as domain from '../domain/businesses.js'
import {
...
export default async function businessRoutes(fastify) {
  ...
  fastify.patch('/businesses/me/registers/:id', { preHandler: borrower }, async (request) => {
    const business = await domain.businessOf(request.user.id)
  ...
  fastify.delete('/businesses/me/registers/:id', { preHandler: borrower }, async (request) => {
    const business = await domain.businessOf(request.user.id)
  ...
  fastify.get('/businesses/:id', { preHandler: fastify.authenticate }, async (request) => {
