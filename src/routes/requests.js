...
import { businessOf } from '../domain/businesses.js'
import { createRequestSchema, fundSchema, listRequestsSchema } from '../validation/requests.js'
...
export default async function requestRoutes(fastify) {
  ...
  fastify.get('/funding-requests/:id', { preHandler: fastify.authenticate }, async (request) => {
    const found = await requests.getRequest(request.params.id, request.user.id)
    return {
      ...serializeRequest(found),
      investors: found.investors.map((i) => ({
        id: i.id,
        name: i.name,
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (result.replayed) return reply.code(200).send({ replayed: true })
  ...
  fastify.delete('/fundings/:id', { preHandler: fastify.guard(['invest']) }, async (request) =>
