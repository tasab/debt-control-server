...
import { currencies, exchangeRates, fxQuotes } from '../../db/schema/index.js'
import { config } from '../config.js'
import { errors } from '../errors.js'
import { divFloor, newId, parseAmount } from '../money/amount.js'
import { postTransaction } from '../money/ledger.js'
import { platformFx, userWallet } from '../money/accounts.js'
...
export async function currentRates(tx = db) {
  const rows = await tx
    .select()
    .from(exchangeRates)
...
async function rateFor(code, tx = db) {
  const [row] = await tx
    .select()
    .from(exchangeRates)
...
async function exponents(tx = db) {
 ...
 */
export function convertMinor({ amount, rate, direction, expFrom, expTo }) {
 ...
 */
export async function quote({ userId, from, to, amountFrom }) {
 ...
 */
export async function execute({ userId, quoteId, idempotencyKey }) {
 ...
 */
export async function ingestRates(rates, { sourceId, log = console }) {
  const accepted = []
