...
import { feePolicies } from '../../db/schema/index.js'
import { applyBpsCeil, max, min } from './amount.js'
 ...
 */
export async function findPolicy(kind, currency, at = new Date(), tx = db) {
...
export function computeFee(policy, amount) {
 ...
 */
export async function previewFee({ kind, currency, amount, at = new Date() }, tx = db) {
