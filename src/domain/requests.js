...
import { userHold, userWallet } from '../money/accounts.js'
import { decodeCursor, encodeCursor } from '../serialize.js'
import { scoreBusiness } from './scoring.js'
import { disburseLoan } from './loans.js'
import { transition } from './requestState.js'

export async function createRequest(businessId, input) {
...
export async function listRequests(query = {}) {
  ...
  return {
    ...
    nextCursor:
      rows.length > limit
        ? encodeCursor({ createdAt: page.at(-1).request.createdAt, id: page.at(-1).request.id })
        : null,
  }
}

const GRADES = ['D', 'C', 'B', 'A']
const gradeAtLeast = (grade, minimum) => GRADES.indexOf(grade) >= GRADES.indexOf(minimum)

export async function getRequest(requestId, viewerId = null) {
  const [row] = await db
 ...
 */
export async function fundRequest({ requestId, investorId, amount, idempotencyKey }) {
  ...
  const outcome = await db.transaction(async (tx) => {
    ...
    const request = (locked.rows ?? locked)[0]
...
export async function cancelFunding({ fundingId, investorId }) {
...
export async function cancelRequest({ requestId, businessId }) {
  return db.transaction(async (tx) => {
...
export async function refundAllHolds(tx, request, reason) {
  const held = await tx
    .select()
    .from(fundings)
...
async function releaseHold(tx, { funding, currency, type, comment }) {
 ...
 */
export async function settleExpiredRequests(log = console) {
  const due = await db
    .select()
    .from(fundingRequests)
