...
export interface Rating {
  score: number
  grade: string
  factors: Record<FactorKey, { value: number; weight: number }>
}
import { businesses, loans, repayments } from '../../db/schema/index.ts'
/**
...
export async function ratingFor(userId: string): Promise<{ grade: string; score: number } | null> {
...
export async function scoreBusiness(
  business: Pick<Business, 'id' | 'isVerified' | 'createdAt'>,
): Promise<Rating> {
  ...
  const score = Math.round(
    (Object.entries(WEIGHTS) as Array<[FactorKey, number]>).reduce(
      (acc, [key, weight]) => acc + factors[key] * weight,
      0,
    ),
  )
  ...
  return {
    ...
    factors: Object.fromEntries(
      (Object.entries(factors) as Array<[FactorKey, number]>).map(([key, value]) => [
        key,
...
async function businessAssetsRough(businessId: string): Promise<Money> {
  ...
  const value = ((rows.rows ?? rows)[0] as { total?: string })?.total ?? 0
  return BigInt(value)
}
