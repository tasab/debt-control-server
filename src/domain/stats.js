...
import { divFloor } from '../money/amount.js'
 ...
 */
export async function balanceHistory({ userId, from, to, currency }) {
...
export async function portfolio(userId) {
  ...
  const interestNet = BigInt(received.interest) - BigInt(received.fee)
  return {
    baseCurrency: base,
    activePrincipal: activeBase,
    totalDeployed: deployedBase,
    interestReceived: BigInt(received.interest),
    feesPaid: BigInt(received.fee),
    principalReturned: BigInt(received.principal),
 ...
 */
export async function writeSnapshots(date = new Date(), log = console) {
