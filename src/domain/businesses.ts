...
import { businessBalances } from '../money/balances.ts'
import { postTransaction } from '../money/ledger.ts'
import { businessWallet, userWallet } from '../money/accounts.ts'
...
export async function dashboard(businessId: string) {
  ...
  const bump = (currency: string, key: 'wallets' | 'registers' | 'debt', value: Money) => {
