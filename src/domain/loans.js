...
import { businessWallet, platformFee, userHold, userWallet } from '../money/accounts.js'
import { computeFee, findPolicy } from '../money/fees.js'
import { transition } from './requestState.js'
 ...
 */
export function interestFor({ principal, rateAnnualBps, days }) {
 ...
 */
export async function disburseLoan(requestId) {
  return db.transaction(async (tx) => {
    const locked = await tx.execute(
      sql`SELECT * FROM funding_requests WHERE id = ${requestId} FOR UPDATE`,
    )
    const raw = (locked.rows ?? locked)[0]
    ...
    const request = {
      ...
      rateAnnualBps: raw.rate_annual_bps,
      termDays: raw.term_days,
      repaymentType: raw.repayment_type,
    }
 ...
 */
export function buildSchedule({ loan, now = new Date() }) {
  const rows = []
  const periodDays = 30
 ...
 */
export async function repay({ loanId, userId, amount, idempotencyKey }) {
...
async function applyToSchedule(tx, loanId, { payPrincipal, payInterest }) {
  const open = await tx
    .select()
    .from(repayments)
...
export async function listLoans({ userId, role, status }) {
  if (role === 'borrower') {
...
export async function getLoan(loanId, viewerId) {
  ...
  if (business.ownerUserId !== viewerId && !share) throw errors.notFound('Позику')
...
async function decorate(loan, { viewerId, share } = {}) {
  const [next] = await db
    .select()
    .from(repayments)
 ...
 */
export async function accrueInterest(now = new Date(), log = console) {
